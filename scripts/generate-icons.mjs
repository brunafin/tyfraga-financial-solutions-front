...
async function generateCoinIcon(svgPath, size) {
  return renderSvg(svgPath, size, size);
}

async function generateMaskableIcon(svgPath, size) {
  const safeZone = Math.round(size * 0.72);
  const icon = await renderSvg(svgPath, safeZone, safeZone);
...
async function main() {
  ...
  console.log("Gerando ícones a partir dos SVGs...\n");

  generateFaviconSvg(logoSvg, join(root, "public/favicon.svg"));

  const outputs = [
  [await generateCoinIcon(logoSvg, 48), join(root, "public/favicon.png")],
  [await generateCoinIcon(logoSvg, 96), join(root, "src/assets/logo.png")],
  [await generateCoinIcon(logoSvg, 180), join(root, "public/apple-touch-icon.png")],
  [await generateCoinIcon(logoSvg, 192), join(root, "public/pwa-192x192.png")],
  [await generateCoinIcon(logoSvg, 512), join(root, "public/pwa-512x512.png")],
